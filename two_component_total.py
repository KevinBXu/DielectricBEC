...
class TwoComponentGas():
    ...
    def __init__(self, element: str = "87Rb", N_particles = (1e6, 1e6),
        ...
        if (N_particles[0] != int(N_particles[0])) or (N_particles[1] != int(N_particles[1])):
            raise TypeError("The number of particles must be an integer")
        #: int: The number of particles in the gas.
        self.N_particles = (int(N_particles[0]), int(N_particles[1]))
        ...
        self._psi = torch.zeros((2, *self.X.size()))
        self._psik = torch.zeros((2, *self.Kx.size()))
        # Specifies the last updated wave function (psi or psik)
        self._updated_wavefunction = None

    def normalize_components(self, psi):
        norms = torch.sqrt(
            torch.sum(torch.abs(psi) ** 2, dim=(-2, -1)) * self.dx * self.dy
        )

        psi = psi / norms[:, None, None]

        fractions = self.N_particles_components / torch.sum(self.N_particles_components)

        psi = psi * torch.sqrt(fractions)[:, None, None]
    ...
    def psi(self):
        """
        Two-component real-space wavefunction.

        Shape:
            self.psi.shape == (2, Nx, Ny)

        where
            self.psi[0] = component 1
            self.psi[1] = component 2
        """

        if self._updated_wavefunction == UPDATED_PSIK:
            psi_full = iftn(self._psik)

            # TorchGPE pads the spatial grid before Fourier transforming.
            # We crop only the last two dimensions, keeping the component axis.
            start_x = self.N_grid // 2
            stop_x = start_x + self.N_grid

            start_y = self.N_grid // 2
            stop_y = start_y + self.N_grid

            self._psi = psi_full[:, start_x:stop_x, start_y:stop_y]
    ...
    def psi(self, value):
        """
        Set two-component real-space wavefunction.
        ...
        if value.ndim != 3:
            raise ValueError("psi must have shape (2, Nx, Ny).")

        if value.shape[0] != 2:
            raise ValueError("First axis of psi must have size 2 for two components.")

        if value.shape[1:] != self.X.shape:
            raise ValueError(
                f"psi spatial shape must match self.X.shape = {self.X.shape}, "
        ...
        self._psi = self.normalize_components(value)
    ...
    def psik(self):
        """
        Two-component momentum-space wavefunction.

        Shape:
            self.psik.shape == (2, Nk, Nk)

        where
            self.psik[0] = component 1 in momentum space
            self.psik[1] = component 2 in momentum space
        """

        if self._updated_wavefunction == UPDATED_PSI:
            # Pad only the last two spatial dimensions.
            # The first dimension is the component index and is unchanged.
            padded_psi = pad(
                self._psi,
                (
                    self.N_grid // 2,
    ...
    def psik(self, value):
        """
        Set two-component momentum-space wavefunction.
        ...
        if value.ndim != 3:
            raise ValueError("psik must have shape (2, Nk, Nk).")

        if value.shape[0] != 2:
            raise ValueError("First axis of psik must have size 2 for two components.")

        Nk = self.N_grid + 2 * (self.N_grid // 2)

        if value.shape[1:] != (Nk, Nk):
            raise ValueError(
                f"psik spatial shape must be ({Nk}, {Nk}), "
                f"but got {value.shape[1:]}."
            )
        ...
        self._psik = self.normalize_components_k(value)
